...
# init_db() is now imported from db.py
...
def update_process_state(process_type, running, command=''):
    ...
    if db_type == 'postgresql':
        cursor.execute('''
            INSERT INTO process_states (process_type, running, command, start_time, last_updated)
        ...
        ''', (process_type, running, command, start_time))
    else:
        cursor.execute('''
...
def get_process_state(process_type):
    ...
    conn, db_type = get_db_connection()
    cursor = conn.cursor()
    cursor.execute('SELECT running, command, start_time FROM process_states WHERE process_type = %s' if db_type == 'postgresql' else 'SELECT running, command, start_time FROM process_states WHERE process_type = ?', (process_type,))
...
def load_initial_process_states():
    ...
    # Reset process states on startup - processes are dead after restart
    if rclone_state['running']:
        print(f"WARNING: Rclone was running before restart with command: {rclone_state['command']}")
        update_process_state('rclone', False, '')
    if terminal_state['running']:
        print(f"WARNING: Terminal was running before restart with command: {terminal_state['command']}")
        update_process_state('terminal', False, '')

def resume_pending_tasks():
    """Resume tasks that were running before restart (up to 5 retries)."""
    ...
    for task in incomplete_tasks:
        ...
        print(f"Resuming task {task['id']}: {task['command']} (retry {task['retry_count'] + 1}/{task['max_retries']})")
        resumed_tasks.append({
            'id': task['id'],
            'command': task['command'],
            'status': 'resumed',
        ...
        # Note: Actual task resumption would require re-executing the command
        # For now, we mark it as failed since we can't re-attach to old processes
        update_task_status(task['id'], 'failed', increment_retry=True)
    
    return resumed_tasks

# Store resumed tasks for notification
resumed_tasks_on_startup = []

with app.app_context():
    load_initial_process_states()
    init_db()
    resumed_tasks_on_startup = resume_pending_tasks()
    if resumed_tasks_on_startup:
        print(f"Found {len(resumed_tasks_on_startup)} tasks to resume on startup")
...
def get_failed_login_attempts(username, time_window_minutes):
    ...
    if db_type == 'postgresql':
        cursor.execute('''
            ...
            WHERE username = %s AND success = FALSE AND timestamp >= %s
...
@app.route('/get-resumed-tasks', methods=['GET'])
@login_required
def get_resumed_tasks():
    """Returns tasks that were resumed on startup for notification."""
    global resumed_tasks_on_startup
    tasks = resumed_tasks_on_startup.copy()
    # Clear after first read so notification only shows once
    resumed_tasks_on_startup = []
    return jsonify({
        "status": "success",
        "tasks": tasks,
...
@app.route('/stream-rclone-output')
@login_required
def stream_rclone_output():
    """SSE endpoint for real-time rclone output streaming."""
    def generate():
        last_position = 0
        while True:
            if os.path.exists(LOG_FILE):
                with open(LOG_FILE, 'r', encoding='utf-8') as f:
                    ...
                    if new_content:
                        ...
                        yield f"data: {json.dumps({'output': new_content, 'is_running': rclone_process and rclone_process.poll() is None})}\n\n"
                        last_position = f.tell()
            time.sleep(0.5)  # Check every 500ms
    
    return Response(generate(), mimetype='text/event-stream')

@app.route('/stream-terminal-output')
@login_required
        ...
        while True:
            if os.path.exists(TERMINAL_LOG_FILE):
                with open(TERMINAL_LOG_FILE, 'r', encoding='utf-8') as f:
                    ...
                    new_content = f.read()
                    if new_content:
                        yield f"data: {json.dumps({'output': new_content, 'is_running': terminal_process and terminal_process.poll() is None})}\n\n"
                        last_position = f.tell()
            time.sleep(0.5)
...
def update_command_status_in_db(table_name, command_id, status):
    ...
    if db_type == 'postgresql':
        cursor.execute(f'UPDATE {table_name} SET status = %s WHERE id = %s', (status, command_id))
    else:
...
def _stream_rclone_output_to_log(process, log_file, stop_flag, command_id, task_id=None):
    ...
    # Update task status
    if task_id:
        update_task_status(task_id, final_status.lower())
...
def execute_rclone():
    ...
    two_remote_modes = ["sync", "copy", "move", "check", "cryptcheck", "archive"]
    ...
    if mode in two_remote_modes:
        ...
        if not source or not destination:
            return jsonify({"status": "error", "message": "Source and Destination are required for archive mode."}), 400
        cmd.extend([source, destination])
    ...
    try:
        ...
        task_data = {
            ...
            'order': order,
            'loglevel': loglevel,
            'additional_flags': additional_flags_str,
            'use_drive_trash': use_drive_trash,
        ...
        task_id = save_task(full_command_str, 'rclone', task_data)
        
        # Save to rclone_transfers table for history
        conn, db_type = get_db_connection()
        cursor = conn.cursor()
        if db_type == 'postgresql':
            cursor.execute('''
                ...
                RETURNING id
            ''', (mode, source, destination, serve_protocol, additional_flags_str))
            rclone_command_id = cursor.fetchone()[0]
        else:
            cursor.execute('''
                INSERT INTO rclone_transfers (mode, source, destination, protocol, flags, status)
                VALUES (?, ?, ?, ?, ?, 'Running')
        ...
        # Start a separate thread to consume output, passing command_id and task_id
        threading.Thread(
            target=_stream_rclone_output_to_log,
            args=(rclone_process, LOG_FILE, stop_rclone_flag, rclone_command_id, task_id),
            daemon=True
        ).start()
...
def _stream_terminal_output_to_buffer(process, buffer, stop_flag, command_id, task_id=None):
...
def execute_terminal_command():
    ...
    with terminal_lock:
        ...
        try:
            ...
            task_id = save_task(command, 'terminal', task_data)
            
            # Save to terminal_commands table
            conn, db_type = get_db_connection()
            cursor = conn.cursor()
            if db_type == 'postgresql':
                cursor.execute('''
                    ...
                    RETURNING id
                ''', (command,))
                terminal_command_id = cursor.fetchone()[0]
            else:
                cursor.execute('''
                    INSERT INTO terminal_commands (command, status)
                    VALUES (?, 'Running')
            ...
            threading.Thread(
                ...
                args=(terminal_process, terminal_output_buffer, stop_terminal_flag, terminal_command_id, task_id),
                daemon=True
            ).start()
...
def get_recent_commands():
    ...
    rclone_transfers = [dict(zip(columns, row)) for row in cursor.fetchall()]
    ...
    columns = [desc[0] for desc in cursor.description]
    terminal_commands = [dict(zip(columns, row)) for row in cursor.fetchall()]
...
def delete_rclone_transfer(item_id):
    ...
    if db_type == 'postgresql':
        cursor.execute('DELETE FROM rclone_transfers WHERE id = %s', (item_id,))
    else:
...
def delete_terminal_command(item_id):
    ...
    if db_type == 'postgresql':
        cursor.execute('DELETE FROM terminal_commands WHERE id = %s', (item_id,))
    else:
